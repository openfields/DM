...
# data
jags.data <- list(y=y, elev=elev, fall.flow=fall.flow, Jdate=Jdate, spring.flow=spring.flow, spring.temp=spring.temp, width=width, winter.flow=winter.flow, nSites=72,nYears=15, nAges=2)
...
params <- c("omega.mu", "omega.mean", "p")

# inits
zst <- apply(jags.data$y,1,max)     # observed occurrence as starting values for z
inits <- function() list(z = zst)

# call jags
out1 <- jags(jags.data, params, "dm1.jags", n.chains=nc, n.thin=nt, n.iter=ni, n.burnin=nb, working.directory=getwd())
